...
public class MemberService {
	...
	public void displayMenu() {
		...
		do {
			...
			menuNum = sc.nextInt(); // 번호를 선택해서 입력할것이므로, nextInt
	...
	public String signUp() {
		// 반환형
		// 해당 메서드는 끝나고 호출한 곳을 돌아갈 때
		// String 자료형 값을 가지고 돌아간다.
		
		System.out.println("\n******회원가입******");
		
		System.out.print("아이디 : ");
		String memberId = sc.next();// 메서드 안에 만들어진 '지역변수'
		...
		if( memberPw.equals(memberPw2) ) {// 일치하는 경우
			...
			return "회원 가입 실패 ! (비밀번호 불일치)"; // signUp에 에러나는게 뵈기싫어서 해둔거임
		}
	...
	public String login() {
		...
		if( memberInfo == null) {// 회원가입을 먼저 하지 않은 경우
		...
		// 같지 않으면 " 아읻 또는 비밀번호가 일치하지 않습니다" 리턴
		
		// 조건문. 아이디 비밀번호가 모두 일치할 경우
		
		if( memberId.contains(memberInfo.getMemberId()) && 
	...
	public int updateMember() {
		...
		if (inputPw.equals(loginMember.getMemberPw()) ) {
			...
			return 0;
		//로그인한 회원의 이름, 나이의 정보를 입력받은 값으로 변경 후 1 반환
		// -> 비밀번호가 다를 경우 0 반환
		}
