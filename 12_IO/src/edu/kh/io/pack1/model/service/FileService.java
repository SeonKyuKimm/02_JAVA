...
public class FileService {
	 ...
	 * -> OS관계 없이 똑같은 코드 작성이 가능
	 * 
	 * -> 이 특징을 유지하기 위해 /, \ 둘다 호환이 가능
	 ...
	 * -기준
	 * [windows} : C:/ , D:/
	 * [Linux, Mac] : /Users , / <- 이거 하나인 경우도 있다
	 * 
	 *   
	 * 2) 상대 경로 : 현재 위치를 기준으로 목표까지의 경로를 표기하는 방법
	 *  			  자바스크립트에서 <script src="../js/js01.js">
	 * 
	 * */
	...
	public void method1() {
		...
		// 제일 앞 '/' : 최 상위 폴더 (== 루트, 절대 경로의 기준점)
				
		// C:/io_test/20240222 폴더를 지정할것
		File directory = new File("/io_test/20240222");
		// -> 처음 : 존재하지 않는 폴더를 관리하는 File 객체를 생성한것  
		...
		if( ! directory.exists() ) { // 폴더가 존재하지 않으면
			...
			System.out.println(directory.getName());// 22040222
			System.out.println(directory.getParent());// 전체 경로 반환. \io_test/20240222
	...
	public void method2() {
		...
		// 문자열에서 "\" 표기하는 방법 : "\\" (콘솔에선백슬래시 하나만 표기됨)
		
		File file = new File("C:\\io_test\\20240222\\파일생성.txt");
	
		// 예외가 발생할 우려
		try {
			
			if(!file.exists()) { // file.exists() 존재하지 않으면
				
				if(file.createNewFile() ) {
					System.out.print(file.getName() +"파일이 생성되었습니다.");
				}
	...
	public void method3() {
		...
		// 향상된 for문 -> for( 요소 저장 변수 : 배열| 커렉션){ }
		for( File f : files ) {
			
			// String File.toString() : 지정된 파일과 디렉토리의 절대경로를 
			//  			 			String형태로 반환해준
			// System.out.println( f.toString() ); // 얻어오고 있는지를 확인
			...
			// System.out.println(lastModified); // 1708388653187
			
		 	// java.text.SimpleDateFormat : 간단히ㅏ 날짜 형식을 지정할 수 있는 객체
