...
public class AbstractService {
	...
	public void ex2() {
		...
		// 추상클래스는 객체로 만들 수 없다.
		//	--> 킹치만 " 참조변수" 로는 사용할 수 있다.
		
		// Animal a1 = new Animal(); (x)
		
		// 사람 -> 동물   /   물고기 -> 동물
		//Animal a1 = new Person(); // (O) 다형성 중 업캐스팅 적용
		//Animal a2 = new Fish(); // (O) 다형성 중 업캐스팅 적용
		...
		// 프로그램 실행 시
		// 참조가호 있는 자식 객체의 오버라이딩된 eat(), breath(), monve()
		// 메소드 수행
		// -> 동적바인딩 : 부모 타입 참조변수로 메소드를 호출했지만
		//					자식 타입의 오버라이딩 된 메소드가 수행된다
