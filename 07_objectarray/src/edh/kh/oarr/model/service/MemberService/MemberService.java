package edh.kh.oarr.model.service.MemberService;

import java.util.Scanner;

import edh.kh.oarr.model.vo.Member.Member;
...
public class MemberService {
	...
	public MemberService() { // MemberService의 기본생성자
	...
	public void displayMenu() {
		...
		}while(menuNum != 0); // menuMun이 0이 되면 반복 종료
	}
	
	// memberArr의 비어있는 인덱스 번호를 반환하는 메서드
	// 단, 비어있는 인덱스가 없다면 -1 반환하도록 
	public int emptyIndex() {
		
		// memberArr 배열을 0번 인덱스부터 배열의 끝까지 접근해서
		// 해당 인덱스가 참조하는 값이 null인 경우의 인덱스를 반환
		for(int i = 0; i < memberArr.length; i++) {
			
			if(memberArr[i] == null) {
				return i; // 현재 메서드를 종료하고 호출한 곳으로 i값을 가지고 돌아간다는 의미
			}
		}
		
		// for문을 수행했지만, return이 되지 않은 경우(index에 빈 자리가 없는 경우), 
		//해당 위치 코드가 수행된다.
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] != null) { // 회원 정보가 있을경우
				...
				if( memberArr[i].getMemberId().equals( memberId ) &&
					...
					// 현재 접근중인 memberArr[i] 번째 요소에 저장된 주소를 얕은복사.
	...
	public String selectMember() {
		...
		System.out.println("\n=======회원 정보 조회=======");
		// 1) 로그인 여부 확인 -> 필도 loginMember가 참조하는 객체가 있는지 확인
			if(loginMember == null) {
	...
	public void searchRegion() {
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			//    위에서 입력받은 지역(inputRegion)과 같을 경우 회원의 아이디, 이름 출력
